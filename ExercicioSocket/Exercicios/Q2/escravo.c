...
int* MultiplicaMatrix (int* coluna, int* linha_1, int* linha_2, int* linha_3, int* resultado, int szMatrix) {
...
int main (int argc, char * argv[]) {
	...
	if (recvfrom(soquete_do_resultado, &szMatrix, sizeof(int), 0, (struct sockaddr *) &mestre, &slen) == -1) {
	...
	sendto(soquete_do_resultado, MultiplicaMatrix(vetor_col, vetor_A, vetor_B, vetor_C, vetor_Final, szMatrix), sizeof(int) * szMatrix, 0, (struct sockaddr *) &mestre, slen);
